...
from complexCommands import ripCommands
...
# Time after which to delete a message (if it is intended to be temporary)
cooldownTime = 15

# For Testing purposes
# Set as test guild id and list with single test guild id
TestingGuild = 357309832484880394
TestingGuildList = [357309832484880394]

# Bot owner ID and Bot token
...
async def on_message(message):
	...
	#SwS motto
	if (message.content.startswith('!motto')): 
	...
	if (message.content.startswith('!adult')):
	...
	if (message.content.startswith('!rip')):
		await ripCommands.commandRip(message, client, owner_user)
...
testlist = [discord.OptionChoice("ChoiceOne","DOGE!"),discord.OptionChoice("ChoiceTwo","NO DOGE")]

@client.slash_command(name="test", description = "This is just a test command, nothing more.", guild_ids = TestingGuildList)#, options = options)
async def test(ctx, optone: discord.Option(str,description = "This is the first option we have.",choices=testlist)):
	global options
	embed = discord.Embed(title="embed test")
	print(testlist)
	await ctx.send(content=optone, embeds=[embed])
	await ctx.respond(content="Here is a hidden message, just for you",ephemeral=True)
	#makes changes to the options variable(adding more choices) and updates the command
	testlist.append(discord.OptionChoice("ChoiceThree","THE SECRET DOGE HIDING UNDER YOUR BED!"))
	print(testlist)
	ctx.command.update()
...
ripOptions = [
	{
		...
		"required"		: False,
		"type"			: 3,
		'choices' 		: ripCommands.ripChoicesList()
	}
]
...
@client.slash_command(description = "A random rip phrase.", guild_ids = TestingGuildList, options = ripOptions)
async def rip(
	ctx: discord.ApplicationContext, 
	name: discord.Option(str,"The descriptive name of the new 'rip'",default=None,choices=ripCommands.ripChoicesList())
):
	await ripCommands.commandRandomRip(name, ctx, owner_user)
...
@slash.slash(name="command", description = "Executes the given command, (owner commands start with @).", guild_ids = TestingGuildList, options = commandOptions)
...
@slash.slash(name="say", description = "Bot says the thing.", guild_ids = TestingGuildList, options = sayOptions)
async def command(ctx: SlashContext, say : str):
	await ctx.send("Sorry, you don't have permission for that. Have some sparkles instead (ﾉ◕ヮ◕)ﾉ*:･ﾟ✧")
